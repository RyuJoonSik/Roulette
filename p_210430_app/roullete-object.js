var roulette = {
  ...
  drawRoulette: function() {
    ...
    for (i = 0; i < this.pieceCount; i++) {
      ...
      ctx.arc(canvasCenterX, canvasCenterY, radius, this.angleToRadian(startAngle), this.angleToRadian(endAngle));
      ctx.stroke();
      ctx.fill();
      ctx.save();
      ...
      ctx.translate(canvasCenterX + Math.cos(this.angleToRadian(angle)) * textRadius, canvasCenterY + Math.sin(this.angleToRadian(angle)) * textRadius);
      rotateAngle = (90 + this.changedAngle / 2) + (endAngle / 2) + (i * this.addedAngle/2);
  ...
  stopRoulette: function() {
    console.log(roulette.angleToRadian(roulette.changedAngle));
  ...
  rotateRoulette: function() {
    if (roulette.passedTime < roulette.totalTime) {
      ...
      roulette.changedAngle = roulette.easingOut(roulette.passedTime, 0, roulette.totalSpin, roulette.totalTime);
...
function showDistrictList() {
  ...
  xhr.onload = function() {
    ...
    for (i = 0; i < districts.length; i++) {
      innerHTMLText += '<li class="main-district--list">' + '<strong class="main-district--name">' + districts[i] + '</strong><em class="main-district--number"></em><ul class="sub-district sub-district--unchecked">';
      for (j = 0; j < district[districts[i]].length; j++) {
        innerHTMLText += '<li>' + district[districts[i]][j] + '</li>';
      }
      innerHTMLText += '</ul></li>';
    }
...
districtList.addEventListener('click', function(e) {
  ...
  if (e.target.nodeName === 'STRONG') {
    ...
    if (e.target.classList.contains('sub-district--list--checked')) {
      for (i = 0; i < e.target.parentNode.children.length; i++) {
        if (e.target.parentNode.children[i].classList.contains('sub-district--list--checked')) {
          count++;
        }
      }
      count = count === 0 ? '' : count;
      // e.target.parentNode.previousSibling.textContent.replace('서울특별시', '시');
  ...
  // if (e.target.firstElementChild === null) {
    ...
    // } else {
      // 하위 행정구역
      // index = roulette.districts.indexOf(e.target.innerText);
      // roulette.districts.splice(0, 1);
      // roulette.initRandom();
      // roulette.drawCanvas();
      // roulette.drawRoulette();
    // }
  // } else {
    // e.target.classList.add('main-district--list--checked');
  //   e.target.style.color = 'blue';
  //   e.target.firstElementChild.classList.toggle('sub-district--unchecked');
...
// canvas
// searchBtn.addEventListener('click', showDistrictList);
// districtList.classList.toggle('district--list--deactivate');
